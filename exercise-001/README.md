...
```sh
git status                      # zeigt ob Veränderungen im Branch vorgenommen wurden
git branch -a                   # ...
git remote -v                   # ...
git fetch --all                 # ...
git checkout -b solution-001    # erstellt einen neuen Branch
git add -p                      # zeigt, welche Zeilen hinzugefügt oder herausgenommen wurden und wie man mit 
                                # der Veränderung verfahren möchte
git commit -m "wählbare Nachricht zum commit"   # commit der veränderten Sachen
...
```
